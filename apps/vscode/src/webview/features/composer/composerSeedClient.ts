...
import { setDraft, type DraftAuthority } from "./composerDraftSync";
import { composerFocusTick } from "../../state/sessionViewStore.svelte";

let appliedComposerSeedId: string | null = null;

export function applyComposerSeed(session: SessionViewModel | null, authority: DraftAuthority): boolean {
  const seed = session?.composerSeed;
  if (!session || !seed || appliedComposerSeedId === seed.id) return false;
  appliedComposerSeedId = seed.id;
  setDraft(session.id, authority, { text: seed.text, images: seed.images.map(seedImage) });

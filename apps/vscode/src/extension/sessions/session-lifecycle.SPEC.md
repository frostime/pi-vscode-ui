...
A new local session is temporary until Pi accepts its first non-empty prompt or the user renames it. Temporary sessions are visible but not persisted; selecting/creating/resuming another session closes the selected temporary session without confirmation unless it has ever been projected into a Session Tab. Opening a temporary Session in a tab retains it for the remainder of that temporary lifetime, including after tab closure; prompt acceptance or rename commits it normally. Resumed sessions are never temporary, and closing a temporary session does not delete Pi-created files. Automatic fork naming and `/compact` do not commit it.

...
- Markdown HTTPS images require an explicit load action. Loaded transcript images preserve aspect ratio, stay within message width and a bounded viewport-relative height, and open a keyboard-accessible Lightbox unless wrapped by a Markdown link. Explicit titles, or concise alt text when no title exists, appear as centered captions without card chrome; load failures and SVG cleaning warnings remain inline and perceivable without color.

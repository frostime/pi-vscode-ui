...
- 本地文件必须是普通文件，并受当前图片附件字节限制约束，默认上限为 10 MiB。
- 可取得固有尺寸的位图受最大单边 16,384 px、最大总像素 40,000,000 px 限制。
...
- 显式 Markdown title 显示为图片下方的居中图注，并继续提供悬停提示。
- alt 不在成功加载后重复显示；它用于无障碍名称和失败回退。
...
- `![alt](./image.png "title")` 能相对于 Session 工作目录显示受支持图片，title 居中显示且没有整体外框。

...
- SVG 与其他图片一样受文件字节限制；其显示尺寸由 Webview CSS 控制，不额外推断 SVG 内部图形结构。
- 本地和 `data:` SVG 在 Webview 中删除 `<script>` 元素后才作为 Blob 图片显示。SVG 不会作为原始 DOM 注入消息。
- SVG 删除脚本后仍可显示时，图片下方显示“已移除 SVG 脚本”警告；SVG 无法解析时显示错误占位。
...
- 普通消息内图片最大宽度为消息可用宽度的 92% 且不超过 640px，最大高度为 `min(400px, 55vh)`；窄视图不得产生水平滚动。
...
- 本地和 `data:` SVG 中的 `<script>` 不会进入最终的 Blob 图片；发生删除时可见警告，无法解析时显示阻止状态。外部资源和其他 SVG 内容不在第一版清理范围内。
